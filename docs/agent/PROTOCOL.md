...
**Deferred action — remove `--allow-base-missing` after the infra PR merges. RESOLVED 2026-07-30.** `.github/scripts/phase_gate_check.py`'s `--allow-base-missing` flag (added 2026-07-30, during the first real phase-gate run) let the ratchet check skip gracefully when the base branch's `coverage.json`/`report.xml` didn't exist yet — a one-time bootstrapping case, since the PR that introduced `--cov`/`--junit-xml` to `scripts/test.sh` was necessarily the first PR where the base branch (`feature/phase2-backtest`) didn't have those files. That PR (GitHub PR #7) merged 2026-07-30; `feature/phase2-backtest` now has the instrumented `test.sh`, and the flag has been removed from `phase-gate.yml`'s checker invocation in the same commit that added this note. See the 2026-07-30 change-log entry for the full validation story.
...
| 2026-07-30 | **First real-world validation of the phase-gate CI**, via a dedicated infra PR (`feature/agent-protocol-infra` → `feature/phase2-backtest`, GitHub PR #7) carrying this session's build (PROTOCOL.md, spec template, pre-push hook, `protocol_lib.py`, updated `test.sh`, phase-gate workflow + checker). Surfaced and fixed three real issues sandbox testing couldn't catch: (1) a download/placement mixup silently swapped `phase-gate.yml`'s content with `phase_gate_check.py`'s, causing a GitHub schema-parse failure (`Invalid workflow file`, line 2 col 1) — fixed by re-placing the correct file and verifying content before every commit going forward; (2) the signed-off `scripts/test.sh` diff similarly failed to land on the first push (old unmodified script committed instead), causing a missing `coverage.json` — fixed the same way; (3) a genuine structural gap: this very PR is what introduces `--cov`/`--junit-xml` to `test.sh`, so `feature/phase2-backtest` (the base/ratchet-comparison branch) couldn't produce `coverage.json`/`report.xml` on its first phase-gate run, since it didn't have the instrumented script yet. Added a `--allow-base-missing` flag to `phase_gate_check.py` (required=False path, PR side still always required) to skip the ratchet gracefully in this one-time case; smoke-tested against 4 new synthetic scenarios (happy path unaffected, missing-base fails without the flag, passes gracefully with it, PR-side-missing always hard-fails regardless of the flag) before shipping. PR #7 went green (770 tests passed, 97.39% coverage) and merged into `feature/phase2-backtest`. Post-merge fast-follow (this commit): removed `--allow-base-missing` now that the base branch has the instrumented `test.sh`, per the deferred action recorded above. **Net result: the phase-gate CI is now validated end-to-end against the real repo, not just synthetic fixtures.** The pre-push hook (§7) remains validated only against synthetic scratch repos — a real-repo validation (task-00 smoke test) is the planned next step. |